...
public class SettingsForm extends Canvas implements CommandListener
{
    ...
    public void keyPressed(int keyCode)
    {
        ...
        if(act==Canvas.UP && selectedRow!=1)
            selectedRow--;
        else if(act == Canvas.DOWN && selectedRow != number)
            selectedRow++;
        repaint();
    }
    private void drawMenu(Graphics g)
    {
        ...
        for(int i = 1; i <= number; i++)
        {
            ...
            g.setColor(0, 0, 0);
            MLT.setText(size+8, size*(i-1)+13-shift, width-size-8,size*i-13, str[i-1]);
            MLT.DrawMultStr();
            g.setColor(120, 120, 120);
            MLT.setText(size+10, size*(i-1)+15-shift, width-size-8,size*i-15, str[i-1]);
            MLT.DrawMultStr();
        }
    }
